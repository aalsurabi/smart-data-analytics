...
test('test', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: 'Bericht Exportieren' }).click();

  await page.screenshot({ path: 'generated_screenshots/few_shot/27_5.png' });
  const htmlContent = await page.content();
  writeFileSync('generated_html/few_shot/27_5.html', htmlContent);
